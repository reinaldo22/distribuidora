...
class SeparateOrderController {

    public async separate(req: Request, res: Response) {

        const { id } = req.params;

        const processService = new ProcessOrderService();

        const statusExists = await getRepository(Status).findOne(2);
        if (!statusExists) {
...
export default new SeparateOrderController();